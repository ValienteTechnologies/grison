"""FROZEN FOR MIGRATION — this is a byte-for-byte copy of the pre-rework converter
(``grison/markdown/converter.py`` as of commit ``ceb8ea3`` / release v0.3.2, the
last hand-rolled-regex version before the markdown-it-py rewrite), kept here ONLY
so :func:`grison.migrate.bodies.migrate_body_v1` can run an existing workspace's
markdown bodies through it. Do not fix bugs in this file, do not add features to
it, do not import it from anywhere except ``grison/migrate/``: every quirk here —
including the four defects the rework fixed (link URLs containing ``)``, no
backslash escapes, no CommonMark hard line break handling, multi-block list items
not handled) — is exactly what an existing workspace's ``html_to_md`` output
looked like, which is precisely what the migration needs to reproduce and then
re-read with the new converter. The only change from the original file is this
notice; the code below is otherwise verbatim (self-contained already — it had no
``grison.*`` imports to adjust).
