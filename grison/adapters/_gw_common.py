"""Shared Ghostwriter report/note lookups (context, slugging, evidence-ref
resolution) both :mod:`grison.adapters.gw_report` and :mod:`grison.adapters.gw_notes`
need — never imported by :mod:`grison.engine` itself (see ``tests/test_engine_no_leak.py``).
"""
...
class IndexRefResolver:
    ...
    :mod:`grison.adapters.gw_evidence` — a separate, not-yet-landed adapter — owns);
    an unresolved reference is never fatal by itself (see the converter's own
    "unresolved references" handling), so this is a safe, honest floor to build on
    rather than a stub: PUSH of an unknown reference is a clear ``ConverterError``
    (surfaced by the validator as ``REP-001``, per the rework's brief D1/D+ENGINE.md),
    and PULL of one round-trips as a visible placeholder instead of failing.
    """
