...
def _print_parse_summary(summary: ParseSummary, out_dir: Path, *, dry_run: bool) -> None:
    ...
    # file-level failure (unreadable/unrecognized/bad encoding/parser raised;
    # see ParseSummary.file_errors) is not the same claim as a finding-level one
    # (a finding that DID parse but failed validation or failed to write; see
    # ParseSummary.finding_errors). Mixing them into one "N finding(s) failed
    # validation" block used to mislabel every file-level failure, and printing
    # `skipped_files` AND `errors` both used to print each file-level failure
    # twice.
    if summary.file_errors:
        typer.secho(f"{len(summary.file_errors)} file(s) could not be parsed:", fg=typer.colors.RED)
        for e in summary.file_errors:
            typer.echo(f"  - {e}")
