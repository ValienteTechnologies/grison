...
def test_parses_findings() -> None:
    ...
    findings = NmapScanner().parse(load("nmap/nmap_sample.xml"), ImportOptions())
...
def test_severity_filter_excludes_info_findings() -> None:
    ...
    findings = NmapScanner().parse(load("nmap/nmap_sample.xml"), opts)
    assert findings == []


def test_no_severity_filter_still_returns_findings() -> None:
    opts = ImportOptions(severity_filter=None)
    findings = NmapScanner().parse(load("nmap/nmap_sample.xml"), opts)
    assert len(findings) == 1
