...
def test_clean() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H1, base_hash=H1) is Outcome.CLEAN
...
def test_repair_when_base_disagrees_but_sides_match() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H1, base_hash=BASE) is Outcome.REPAIR
...
def test_clean_when_sides_match_and_no_base() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H1, base_hash=None) is Outcome.CLEAN


# Row 2: present+indexed, remote present, base set, L != base, R == base -> PUSH
def test_push() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=BASE, base_hash=BASE) is Outcome.PUSH


# Row 3: present+indexed, remote present, base set, L == base, R != base -> PULL
def test_pull() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=BASE, remote_hash=H2, base_hash=BASE) is Outcome.PULL


# Row 4: present+indexed, remote present, L != R, both != base (base set) -> COLLISION
def test_collision_both_diverged_from_base() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H2, base_hash=BASE) is Outcome.COLLISION


# Row 4 variant: L != R, base is None (no state at all) -> COLLISION
def test_collision_no_base_and_sides_differ() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H2, base_hash=None) is Outcome.COLLISION


# Row 5: present, unindexed -> CREATE
def test_create() -> None:
    assert classify(indexed=False, local_present=True, remote_present=False,
                    local_hash=H1, remote_hash=None, base_hash=None) is Outcome.CREATE
...
def test_delete_remote() -> None:
    assert classify(indexed=True, local_present=False, remote_present=True,
                    local_hash=None, remote_hash=BASE, base_hash=BASE) is Outcome.DELETE_REMOTE


# Row 7: missing, indexed, remote present, base set, R != base -> COLLISION
def test_collision_deleted_locally_edited_remotely() -> None:
    assert classify(indexed=True, local_present=False, remote_present=True,
                    local_hash=None, remote_hash=H2, base_hash=BASE) is Outcome.COLLISION
...
def test_delete_local() -> None:
    assert classify(indexed=True, local_present=True, remote_present=False,
                    local_hash=BASE, remote_hash=None, base_hash=BASE) is Outcome.DELETE_LOCAL


# Row 9: present, indexed, remote gone, base set, L != base -> COLLISION
def test_collision_edited_locally_deleted_remotely() -> None:
    assert classify(indexed=True, local_present=True, remote_present=False,
                    local_hash=H1, remote_hash=None, base_hash=BASE) is Outcome.COLLISION
...
def test_collision_remote_gone_no_base() -> None:
    assert classify(indexed=True, local_present=True, remote_present=False,
                    local_hash=H1, remote_hash=None, base_hash=None) is Outcome.COLLISION
...
def test_forget() -> None:
    assert classify(indexed=True, local_present=False, remote_present=False,
                    local_hash=None, remote_hash=None, base_hash=BASE) is Outcome.FORGET
    assert classify(indexed=True, local_present=False, remote_present=False,
                    local_hash=None, remote_hash=None, base_hash=None) is Outcome.FORGET


# Row 11: absent locally, remote present, unindexed -> PULL_NEW
def test_pull_new() -> None:
    assert classify(indexed=False, local_present=False, remote_present=True,
                    local_hash=None, remote_hash=H1, base_hash=None) is Outcome.PULL_NEW


def test_raises_for_nothing_to_classify() -> None:
    with pytest.raises(ValueError, match="nothing to classify"):
        classify(indexed=False, local_present=False, remote_present=False,
...
def test_force_local_on_ordinary_collision_pushes() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    ...
                    force_local=True) is Outcome.PUSH


def test_force_remote_on_ordinary_collision_pulls() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H2, base_hash=BASE,
                    force_remote=True) is Outcome.PULL


def test_force_local_on_deleted_locally_edited_remotely_deletes_remote() -> None:
    assert classify(indexed=True, local_present=False, remote_present=True,
                    local_hash=None, remote_hash=H2, base_hash=BASE,
                    force_local=True) is Outcome.DELETE_REMOTE
...
def test_force_remote_on_edited_locally_deleted_remotely_deletes_local() -> None:
    assert classify(indexed=True, local_present=True, remote_present=False,
                    local_hash=H1, remote_hash=None, base_hash=BASE,
                    force_remote=True) is Outcome.DELETE_LOCAL
...
def test_force_flags_have_no_effect_outside_a_collision() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    ...
                    force_local=True, force_remote=True) is Outcome.CLEAN
...
def test_read_only_never_pushes() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    ...
                    read_only=True) is Outcome.INVALID


def test_read_only_never_creates() -> None:
    assert classify(indexed=False, local_present=True, remote_present=False,
                    local_hash=H1, remote_hash=None, base_hash=None,
                    read_only=True) is Outcome.INVALID
...
def test_read_only_still_pulls() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    ...
                    read_only=True) is Outcome.PULL


def test_read_only_still_pulls_new() -> None:
    assert classify(indexed=False, local_present=False, remote_present=True,
                    local_hash=None, remote_hash=H1, base_hash=None,
                    read_only=True) is Outcome.PULL_NEW


def test_read_only_still_deletes_local() -> None:
    assert classify(indexed=True, local_present=True, remote_present=False,
                    local_hash=BASE, remote_hash=None, base_hash=BASE,
                    read_only=True) is Outcome.DELETE_LOCAL


def test_read_only_still_cleans() -> None:
    assert classify(indexed=True, local_present=True, remote_present=True,
                    local_hash=H1, remote_hash=H1, base_hash=H1,
                    read_only=True) is Outcome.CLEAN
