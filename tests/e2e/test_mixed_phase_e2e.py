"""End-to-end: all three phases (findings, reports, methodology) in one
``grison sync`` invocation, mixed outcomes.
...
itself makes the overall exit code 1 — but the reports and methodology phases are
fully isolated from that failure (``grison.cli._run_phase`` catches per phase) and
still run, and still print their own clean summary lines.
"""
...
def test_all_three_phases_run_with_mixed_outcomes(run_grison, gw_server, bs_server):
    ...
    # methodology: succeeded cleanly, isolated from both phases before it
